...
RSpec.describe Rswag::Specs::SwaggerFormatter do
  ...
  describe '#stop' do
    ...
    context 'with oauth3 upgrades' do
      ...
      it 'removes remaining consumes/produces' do
        expect(doc2[:paths]['/path/'][:get].keys).to eql(%i[summary tags parameters requestBody])
      end

      it 'duplicates params in: :body to requestBody from consumes list' do
        expect(doc2[:paths]['/path/'][:get][:parameters]).to eql([{ in: :headers }])
        expect(doc2[:paths]['/path/'][:get][:requestBody]).to eql(content: {
    ...
    context 'with oauth3 formData' do
      ...
      it 'removes remaining consumes/produces' do
        expect(doc2[:paths]['/path/'][:post].keys).to eql(%i[summary tags parameters requestBody])
      end

      it 'duplicates params in: :formData to requestBody from consumes list' do
        expect(doc2[:paths]['/path/'][:post][:parameters]).to eql([{ in: :headers }])
        expect(doc2[:paths]['/path/'][:post][:requestBody]).to eql(content: {
    ...
    context 'with descriptions on the body param' do
      ...
      it 'puts the description in the doc' do
        expect(doc2[:paths]['/path/'][:post][:requestBody][:description]).to eql('description')
      end
    end
