...
RSpec.describe Rswag::Specs::RequestFactory do
    ...
    context 'basic auth' do
      ...
      context 'openapi 3.0.1 upgrade notice' do
        ...
        it 'warns the user to upgrade' do
          expect(request[:headers]).to eq('HTTP_AUTHORIZATION' => 'Basic foobar')
          expect(ActiveSupport::Deprecation).to have_received(:warn).with(<<~WARNING.squish)
            Rswag::Specs: WARNING: securityDefinitions is replaced in OpenAPI3!
    ...
    context 'referenced parameters' do
      ...
      context 'openapi 3.0.1 upgrade notice' do
        ...
        it 'warns the user to upgrade' do
          expect(request[:path]).to eq('/blogs?q1=foo')
          expect(ActiveSupport::Deprecation).to have_received(:warn).with(<<~WARNING.squish)
            Rswag::Specs: WARNING: #/parameters/ refs are replaced in OpenAPI3!
            Rename to #/components/parameters/
          WARNING
          expect(ActiveSupport::Deprecation).to have_received(:warn).with(<<~WARNING.squish)
            Rswag::Specs: WARNING: parameters is replaced in OpenAPI3!
