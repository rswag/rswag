    ...
    class ResponseValidator
      ...
      def validate!(metadata, response)
        ...
        if swagger_docs.is_a?(Array)
          swagger_docs.each { |swagger_doc| validate(swagger_doc, metadata, response) }
        else
          validate(swagger_docs, metadata, response)
        end
      end

      private

      def validate(swagger_doc, metadata, response)
      ...
      def validate_body!(metadata, doc, body)
        response_schema = metadata[:response][:schema]
        return if response_schema.nil?

        version = @config.get_openapi_spec_version(doc)
        schemas = definitions_or_component_schemas(doc, version)
